...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Graphique pour TOUS les sites : affiche la part de succès de chaque site
        fig = px.pie(
            spacex_df, 
            values='class', 
            names='Launch Site', 
        ...
        # Graphique pour UN site spécifique : affiche le ratio Succès (1) vs Échecs (0)
        ...
        fig = px.pie(
            ...
            color='class',
            color_discrete_map={1: 'green', 0: 'red'} # Optionnel : Vert pour succès, Rouge pour échec
        )
        return fig
...
def get_scatter_chart(entered_site, payload_range):
    # Filtrer d'abord par la plage de masse de la charge utile (Slider)
    low, high = payload_range
    mask = (spacex_df['Payload Mass (kg)'] >= low) & (spacex_df['Payload Mass (kg)'] <= high)
    filtered_df = spacex_df[mask]
    
    if entered_site == 'ALL':
        # Scatter plot pour tous les sites
        fig = px.scatter(
            filtered_df,
            x='Payload Mass (kg)',
            y='class',
        ...
        # Scatter plot filtré pour le site sélectionné
